...
Pass Vite backend URLs at image build time when the defaults are not suitable:
...
```bash
node scripts/build-images.mjs --build-arg VITE_FEDERATION_LEARNING_SUPPORT_API_URL=http://localhost:8080
```
