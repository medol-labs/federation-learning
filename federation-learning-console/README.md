...
./mvnw -pl federation-learning-console -am spring-boot:run
```
...
- PostgreSQL host port: `5434`; override with `DB_PORT`
- PostgreSQL database: `federation_learning_console`; override the full connection with `DB_URL`
...
```bash
cd ..
MEDOL_AXON_EVENT_STORAGE=inmemory AXON_SERVER_ENABLED=false ./mvnw -pl federation-learning-console -am spring-boot:run
```
...
docker compose up -d umadb federation-learning-console-postgres./mvnw -pl federation-learning-console -am spring-boot:run
```
...
```bash
cd ..
./mvnw -pl federation-learning-console -am clean verify
```
...
./mvnw -pl federation-learning-console jib:dockerBuild
```

The generated image is `medol/federation-learning-console:0.0.1-SNAPSHOT` and exposes port `8082`.
