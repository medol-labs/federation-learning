...
import { ApproveModelCommandSchema, type ApproveModelCommandInput } from "@/contexts/domain/schemas";
...
export const ModelCatalogApproveModel = () => {
  ...
  async function onSubmit(values: ApproveModelCommandInput) {
    const result = await runFormBehavior<ApproveModelCommandInput>(
      frontendComposition,
      "behavior:model-catalog:approveModel",
      {
      ...defaultValues,
      ...values,
      } as ApproveModelCommandInput,
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.model_catalog.commands.approveModel.label", "Approve Model")} />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("ApproveModel validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.model_catalog.commands.approveModel.fields.approvalNote.label", "Approval Note")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
