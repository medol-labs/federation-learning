...
import { PauseTrainingJobCommandSchema, type PauseTrainingJobCommandInput } from "@/contexts/domain/schemas";

export const TrainingJobDashboardPauseTrainingJob = () => {
  ...
  async function onSubmit(values: PauseTrainingJobCommandInput) {
    const result = await runFormBehavior<PauseTrainingJobCommandInput>(
      frontendComposition,
      "behavior:training-job-dashboard:pauseTrainingJob",
      {
      ...defaultValues,
      ...values,
      } as PauseTrainingJobCommandInput,
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.training_job_dashboard.commands.pauseTrainingJob.label", "Pause Training Job")} />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("PauseTrainingJob validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.training_job_dashboard.commands.pauseTrainingJob.fields.pauseReason.label", "Pause Reason")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
