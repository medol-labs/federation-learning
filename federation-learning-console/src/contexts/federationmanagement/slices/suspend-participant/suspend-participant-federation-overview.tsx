...
export const FederationOverviewSuspendParticipant = () => {
  ...
  async function onSubmit(values: SuspendParticipantCommandInput) {
    const result = await runFormBehavior<SuspendParticipantCommandInput>(
      frontendComposition,
      "behavior:federation-overview:suspendParticipant",
      {
      ...defaultValues,
      ...values,
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("SuspendParticipant validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_overview.commands.suspendParticipant.fields.federationName.label", "Federation Name")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_overview.commands.suspendParticipant.fields.organizationName.label", "Organization Name")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
