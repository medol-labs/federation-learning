...
export const FederationOverviewRemoveParticipant = () => {
  ...
  async function onSubmit(values: RemoveParticipantCommandInput) {
    const result = await runFormBehavior<RemoveParticipantCommandInput>(
      frontendComposition,
      "behavior:federation-overview:removeParticipant",
      {
      ...defaultValues,
      ...values,
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.federation_overview.commands.removeParticipant.label", "Remove Participant")} />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("RemoveParticipant validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_overview.commands.removeParticipant.fields.organizationId.label", "Organization Id")}</FormLabel>
                <ResourceSelect
                  withFormControl
                  ...
                  meta={{
                    ...
                    label: t("resources.federation_overview.commands.removeParticipant.fields.organizationId.label", "Organization Directory"),
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_overview.commands.removeParticipant.fields.removalReason.label", "Removal Reason")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
