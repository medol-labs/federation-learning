...
export const FederationMembershipDirectoryRejectParticipant = () => {
  ...
  async function onSubmit(values: RejectParticipantCommandInput) {
    const result = await runFormBehavior<RejectParticipantCommandInput>(
      frontendComposition,
      "behavior:federation-membership-directory:rejectParticipant",
      {
      ...defaultValues,
      ...values,
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("RejectParticipant validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_membership_directory.commands.rejectParticipant.fields.federationName.label", "Federation Name")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_membership_directory.commands.rejectParticipant.fields.organizationName.label", "Organization Name")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
