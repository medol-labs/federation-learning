...
import { RejectParticipantCommandSchema, type RejectParticipantCommandInput } from "@/contexts/domain/schemas";
import { ResourceMultiSelect, ResourceSelect } from "@/components/refine-ui/form/resource-select";

export const FederationOverviewRejectParticipant = () => {
  ...
  async function onSubmit(values: RejectParticipantCommandInput) {
    const result = await runFormBehavior<RejectParticipantCommandInput>(
      frontendComposition,
      "behavior:federation-overview:rejectParticipant",
      {
      ...defaultValues,
      ...values,
      } as RejectParticipantCommandInput,
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.federation_overview.commands.rejectParticipant.label", "Reject Participant")} />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("RejectParticipant validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_overview.commands.rejectParticipant.fields.federationName.label", "Federation Name")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_overview.commands.rejectParticipant.fields.organizationId.label", "Organization Id")}</FormLabel>
                <ResourceSelect
                  withFormControl
                  ...
                  placeholder={t("resources.federation_overview.commands.rejectParticipant.fields.organizationId.placeholder", "Select Organization Id")}
                  meta={{
                    idField: "organizationId",
                    label: t("resources.federation_overview.commands.rejectParticipant.fields.organizationId.label", "Organization Directory"),
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_overview.commands.rejectParticipant.fields.organizationName.label", "Organization Name")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_overview.commands.rejectParticipant.fields.rejectionReason.label", "Rejection Reason")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
