...
const toAbsoluteBaseUrl = (baseUrl: string) => {
  ...
  return new URL(trimmedBaseUrl, `${browserOrigin()}/`).toString().replace(/\/+$/u, "");
};
...
export function useFileDownload(options: DownloadFileOptions = {}) {
  ...
  const baseUrl = useMemo(() => {
    const moduleName = options.backendModuleName ?? SUPPORT_MODULE_NAME;
    const backendModule = backendModules.find((item) => item.name === moduleName);
