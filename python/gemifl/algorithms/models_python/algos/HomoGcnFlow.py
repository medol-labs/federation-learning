...
class GCN(nn.Module):
    ...
    def __init__(self, feat_dim, hidden_dim1, hidden_dim2, dropout, is_sparse=False):
        ...
        self.W1 = nn.Parameter(flow.FloatTensor(feat_dim, hidden_dim1))
        self.W2 = nn.Parameter(flow.FloatTensor(hidden_dim1, hidden_dim2))
        self.relu = nn.ReLU()
    ...
    def forward(self, x, adj):
        # Layer 1
        support = flow.mm(x, self.W1)
        embeddings = flow.mm(adj, support) if self.is_sparse else flow.mm(adj, support)

        embeddings = self.dropout(embeddings)

        # Layer 2
        support = flow.mm(embeddings, self.W2)
        embeddings = flow.mm(adj, support) if self.is_sparse else flow.mm(adj, support)
...
class GCNNodeCLF(flow.nn.Module):
    def __init__(self, nfeat, nhid, nclass, nlayer, dropout):
        ...
        self.pre = flow.nn.Sequential(flow.nn.Linear(nfeat, nhid))

        self.graph_convs = flow.nn.ModuleList()

        for l in range(nlayer - 1):
            self.graph_convs.append(GCNConv(nhid, nhid))

        self.post = flow.nn.Sequential(flow.nn.Linear(nhid, nclass))
...
class GCNLinkPred(flow.nn.Module):
    ...
    def decode(self, z, pos_edge_index, neg_edge_index):
        edge_index = flow.cat([pos_edge_index, neg_edge_index], dim=-1)
...
class HomoGcnFlow:
    ...
    def get_device(self):
        ...
        if flow.cuda.is_available() and userdevice != "cpu":
            try:
                self.device = flow.device(userdevice)
            except:
                self.device = flow.device("cuda:0")
    ...
    def set_weights(self, parameters):
        ...
        for name, data in parameters.items():
            weight_input[name] = flow.tensor(
                np.reshape(parameters[name][0], parameters[name][1])
            )
    ...
    def init_model(self):
        ...
        self.optimizer = flow.optim.Adam(self.model.parameters(), lr=self.learning_rate)
        self.criterion = flow.nn.BCEWithLogitsLoss(reduction="none")
    ...
    def train_one_epoch(self):
        ...
        for tmp_feature, tmp_label, tmp_adj in zip(
            ...
            # convert to tensor
            tmp_adj = flow.tensor(tmp_adj.todense(), dtype=flow.float32).to(self.device)

            tmp_feature = flow.tensor(tmp_feature, dtype=flow.float32).to(self.device)
            tmp_label = flow.tensor(tmp_label, dtype=flow.float32).to(self.device)
            # tmp_label = tmp_label.to(dtype=flow.float32, non_blocking=True)
            # tmp_label =
    ...
    def evaluate(self, task_type):
        ...
        with flow.no_grad():
            for tmp_feature, tmp_label, tmp_adj in zip(
                self.test_dataloader["x"],
                self.test_dataloader["y"],
                self.test_dataloader["adj"],
            ):
                tmp_adj = flow.tensor(tmp_adj.todense(), dtype=flow.float32).to(
                    self.device
                )

                tmp_feature = flow.tensor(tmp_feature, dtype=flow.float32).to(
                    self.device
                )
    ...
    def dumpfile(self):
        ...
        # flow.save(self.model.state_dict(), self.weight_save)
