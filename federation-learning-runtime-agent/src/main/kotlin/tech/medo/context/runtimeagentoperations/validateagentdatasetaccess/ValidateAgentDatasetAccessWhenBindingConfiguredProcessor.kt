...
class ValidateAgentDatasetAccessWhenBindingConfiguredProcessor(private val commandGateway: CommandGateway) {
    ...
    fun on(event: RuntimeDatasetBindingConfiguredEvent): java.util.concurrent.CompletableFuture<*> =
        commandGateway.send(ValidateAgentDatasetAccessCommand(datasetAccessValidationId = java.util.UUID.randomUUID() /* TODO: provide datasetAccessValidationId */, runtimeDatasetBindingId = event.runtimeDatasetBindingId, datasetId = event.datasetId, runtimeId = event.runtimeId, dataSourceType = event.dataSourceType, host = event.host, port = event.port, url = event.url, databaseName = event.databaseName, schemaName = event.schemaName, tableName = event.tableName, filePath = event.filePath, objectBucket = event.objectBucket, objectPrefix = event.objectPrefix, dataFormat = event.dataFormat, credentialSecretName = event.credentialSecretName)).resultMessage
}
