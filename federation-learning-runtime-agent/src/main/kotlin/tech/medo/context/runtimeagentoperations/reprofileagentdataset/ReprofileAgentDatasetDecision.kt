...
interface ReprofileAgentDatasetDecision {
    fun decide(command: ReprofileAgentDatasetCommand, state: AgentDatasetProfileState, portResult: ReprofileAgentDatasetResult, now: java.time.LocalDateTime): List<Any> {
        ...
        return when (portResult) {
                    is ReprofileAgentDatasetResult.Succeeded -> listOf(AgentDatasetReprofiledEvent(metadataReportId = command.metadataReportId, runtimeDatasetBindingId = command.runtimeDatasetBindingId, datasetId = requireNotNull(state.datasetId) { "datasetId is required from state." }, organizationId = requireNotNull(state.organizationId) { "organizationId is required from state." }, runtimeId = requireNotNull(state.runtimeId) { "runtimeId is required from state." }, featureSchemaId = requireNotNull(state.featureSchemaId) { "featureSchemaId is required from state." }, sampleCount = portResult.sampleCount, featureCount = portResult.featureCount, schemaCompatible = portResult.schemaCompatible, labelCompatible = portResult.labelCompatible, missingValueRate = portResult.missingValueRate, duplicateRate = portResult.duplicateRate, qualityScore = portResult.qualityScore, nonIidScore = portResult.nonIidScore, classBalanceScore = portResult.classBalanceScore))
