...
./mvnw -pl flplatform-backend -am spring-boot:run
```
...
- PostgreSQL host port: `5433`; override with `DB_PORT`
- PostgreSQL database: `flplatform_backend`; override the full connection with `DB_URL`
...
```bash
cd ..
MEDOL_AXON_EVENT_STORAGE=inmemory AXON_SERVER_ENABLED=false ./mvnw -pl flplatform-backend -am spring-boot:run
```
...
docker compose up -d umadb flplatform-backend-postgres./mvnw -pl flplatform-backend -am spring-boot:run
```
...
```bash
cd ..
./mvnw -pl flplatform-backend -am clean verify
```
...
./mvnw -pl flplatform-backend jib:dockerBuild
```

The generated image is `medol/flplatform-backend:0.0.1-SNAPSHOT` and exposes port `8081`.
