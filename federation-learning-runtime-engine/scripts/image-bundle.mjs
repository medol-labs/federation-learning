...
const defaultImageName = 'federation-learning-runtime-engine';
const defaultTar = 'federation-learning-runtime-engine-images.tar';
const defaultDockerfile = './docker/pytorch/Dockerfile';
...
function printPlan() {
    ...
    console.log(`[images] dockerfile: ${dockerfile}`);
...
function ensureDockerfile() {
    if (!existsSync(resolve(root, dockerfile))) {
        fail(`Cannot find Dockerfile: ${dockerfile}`);
    }
}
