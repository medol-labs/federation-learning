...
./mvnw -pl fldictionary-backend -am spring-boot:run
```
...
- PostgreSQL database: `fldictionary_backend`; override the full connection with `DB_URL`
...
```bash
cd ..
MEDOL_AXON_EVENT_STORAGE=inmemory AXON_SERVER_ENABLED=false ./mvnw -pl fldictionary-backend -am spring-boot:run
```
...
docker compose up -d umadb fldictionary-backend-postgres./mvnw -pl fldictionary-backend -am spring-boot:run
```
...
```bash
cd ..
./mvnw -pl fldictionary-backend -am clean verify
```
...
./mvnw -pl fldictionary-backend jib:dockerBuild
```

The generated image is `medol/fldictionary-backend:0.0.1-SNAPSHOT` and exposes port `8080`.
