...
./mvnw -pl flruntime-agent -am spring-boot:run
```
...
- PostgreSQL database: `flruntime_agent`; override the full connection with `DB_URL`
...
```bash
cd ..
MEDOL_AXON_EVENT_STORAGE=inmemory AXON_SERVER_ENABLED=false ./mvnw -pl flruntime-agent -am spring-boot:run
```
...
docker compose up -d umadb flruntime-agent-postgres./mvnw -pl flruntime-agent -am spring-boot:run
```
...
```bash
cd ..
./mvnw -pl flruntime-agent -am clean verify
```
...
./mvnw -pl flruntime-agent jib:dockerBuild
```

The generated image is `medol/flruntime-agent:0.0.1-SNAPSHOT` and exposes port `8082`.
