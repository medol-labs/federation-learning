...
import { RevokeDatasetTrainingApprovalCommandSchema, type RevokeDatasetTrainingApprovalCommandInput } from "@/contexts/domain/schemas";

export const DatasetReadinessRevokeDatasetTrainingApproval = () => {
  ...
  } as unknown as Partial<RevokeDatasetTrainingApprovalCommandInput>;
  ...
  async function onSubmit(values: RevokeDatasetTrainingApprovalCommandInput) {
    const result = await runFormBehavior<RevokeDatasetTrainingApprovalCommandInput>(
      frontendComposition,
      "behavior:dataset-readiness:revokeDatasetTrainingApproval",
      {
      ...defaultValues,
      ...values,
      } as RevokeDatasetTrainingApprovalCommandInput,
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.dataset_readiness.commands.revokeDatasetTrainingApproval.label", "Revoke Dataset Training Approval")} />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("RevokeDatasetTrainingApproval validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.dataset_readiness.commands.revokeDatasetTrainingApproval.fields.revokeReason.label", "Revoke Reason")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
