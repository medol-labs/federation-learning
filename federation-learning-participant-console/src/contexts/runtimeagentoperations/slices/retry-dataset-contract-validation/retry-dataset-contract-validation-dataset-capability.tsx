...
export const DatasetCapabilityRetryDatasetContractValidation = () => {
  ...
  async function onSubmit(values: RetryDatasetContractValidationCommandInput) {
    const result = await runFormBehavior<RetryDatasetContractValidationCommandInput>(
      frontendComposition,
      "behavior:dataset-capability:retryDatasetContractValidation",
      {
      ...defaultValues,
      ...values,
      } as RetryDatasetContractValidationCommandInput,
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.dataset_capability.commands.retryDatasetContractValidation.label", "Retry Dataset Contract Validation")} />
      <Form {...form}>
