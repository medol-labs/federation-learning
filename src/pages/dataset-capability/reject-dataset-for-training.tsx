...
import { RejectDatasetForTrainingCommandSchema, type RejectDatasetForTrainingCommandInput } from "@/domain/schemas";


export const DatasetCapabilityRejectDatasetForTraining = () => {
  ...
  function onSubmit(values: RejectDatasetForTrainingCommandInput) {
    return onFinish({
      ...defaultValues,
      ...values,
    });
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.dataset_capability.commands.rejectDatasetForTraining.label", "Reject Dataset For Training")} />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("RejectDatasetForTraining validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.dataset_capability.commands.rejectDatasetForTraining.fields.rejectionReason.label", "Rejection Reason")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
