...
export const TrainingParticipantEligibilityCancelTrainingJob = () => {
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.training_participant_eligibility.commands.cancelTrainingJob.label", "Cancel Training Job")} />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("CancelTrainingJob validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.training_participant_eligibility.commands.cancelTrainingJob.fields.cancelReason.label", "Cancel Reason")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
