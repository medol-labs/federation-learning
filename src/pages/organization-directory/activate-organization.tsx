...
import { ActivateOrganizationCommandSchema, type ActivateOrganizationCommandInput } from "@/domain/schemas";


export const OrganizationDirectoryActivateOrganization = () => {
  ...
  } as Partial<ActivateOrganizationCommandInput>;

  const { refineCore: { onFinish }, ...form } = useCommandForm<ActivateOrganizationCommandInput, ActivateOrganizationCommandInput>({
    ...
    formProps: {
      defaultValues,
      resolver: zodResolver(ActivateOrganizationCommandSchema) as never,
    },
  });

  function onSubmit(values: ActivateOrganizationCommandInput) {
    return onFinish({
      ...defaultValues,
      ...values,
    });
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("ActivateOrganization validation failed", errors))} className="space-y-8">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.organization_directory.commands.activateOrganization.fields.activationNote.label", "Activation Note")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
