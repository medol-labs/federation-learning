...
export const FederationMembershipDirectoryCreateFederation = () => {
  ...
  return (
    <CreateView>
      <CreateViewHeader title={t("resources.federation_membership_directory.commands.createFederation.label", "Create Federation")} />
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit, (errors) => console.error("CreateFederation validation failed", errors))} className="space-y-8">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_membership_directory.commands.createFederation.fields.federationName.label", "Federation Name")}</FormLabel>
                <FormControl>
                  <Input
                    {...field}
                    value={field.value || ""}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_membership_directory.commands.createFederation.fields.description.label", "Description")}</FormLabel>
                <FormControl>
                  <Textarea
                    {...field}
                    value={field.value || ""}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("resources.federation_membership_directory.commands.createFederation.fields.minimumParticipantCount.label", "Minimum Participant Count")}</FormLabel>
                <FormControl>
                  <Input
                    type="number"
                    {...field}
