# FederationLearningRuntimeAgent
...
- PostgreSQL database: `federation_learning_runtime_agent`; override the full connection with `DB_URL`
...
```bash
cd ..
MEDOL_AXON_EVENT_STORAGE=inmemory AXON_SERVER_ENABLED=false ./mvnw -pl federation-learning-runtime-agent -am spring-boot:run
```
...
```bash
cd ..
./mvnw -pl federation-learning-runtime-agent -am clean verify
```
...
```bash
cd ..
node scripts/build-images.mjs --module federation-learning-runtime-agent
```
...
```bash
cd ..
node scripts/export-images.mjs --module federation-learning-runtime-agent
```
...
```bash
node scripts/import-images.mjs --file federation-learning-runtime-agent-images.tar
...
```bash
cd ..
node scripts/image-bundle.mjs all --module federation-learning-runtime-agent
```
...
The generated image is `medol/federation-learning-runtime-agent:0.0.1-SNAPSHOT` and exposes port `8082`.
