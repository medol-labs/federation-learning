...
@Namespace("automation-training-orchestration-start-training-round")
@Component
class StartRoundWhenEncryptionContextPreparedProcessor(private val commandGateway: CommandGateway) {
    ...
    fun on(event: HomomorphicEncryptionContextPreparedEvent): java.util.concurrent.CompletableFuture<*> =
        if (event.selectedRuntimeCount >= event.minimumNodesPerRound) {
            commandGateway.send(StartTrainingRoundCommand(trainingJobId = event.trainingJobId, federationId = event.federationId, federationName = event.federationName, trainingRunConfigurationId = event.trainingRunConfigurationId, configurationName = event.configurationName, trainingJobObjective = event.trainingJobObjective, featureSchemaId = event.featureSchemaId, featureDomain = event.featureDomain, featureSchemaVersion = event.featureSchemaVersion, roundId = event.roundId, roundNumber = event.roundNumber, selectedOrganizationIds = event.selectedOrganizationIds, selectedRuntimeIds = event.selectedRuntimeIds, selectedOrganizationCount = event.selectedOrganizationCount, selectedRuntimeCount = event.selectedRuntimeCount, minimumNodesPerRound = event.minimumNodesPerRound, maxRounds = event.maxRounds, minimumAccuracy = event.minimumAccuracy, aggregationAlgorithm = null /* TODO: provide aggregationAlgorithm */, secureAggregationRequired = event.secureAggregationRequired, secureAggregationSessionId = event.secureAggregationSessionId, encryptionScheme = event.encryptionScheme, publicKeyVersion = event.publicKeyVersion, publicKeyRef = event.publicKeyRef, encryptedParameterScale = event.encryptedParameterScale)).resultMessage
        } else {
