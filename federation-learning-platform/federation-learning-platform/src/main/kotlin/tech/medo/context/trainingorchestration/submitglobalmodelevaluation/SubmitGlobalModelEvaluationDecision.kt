...
interface SubmitGlobalModelEvaluationDecision {
    fun decide(command: SubmitGlobalModelEvaluationCommand, state: TrainingRoundState, portResult: SubmitGlobalModelEvaluationResult): List<Any> {
        ...
        return when (portResult) {
                    is SubmitGlobalModelEvaluationResult.Succeeded -> listOf(GlobalModelEvaluationSubmittedEvent(trainingJobId = command.trainingJobId, trainingRunConfigurationId = command.trainingRunConfigurationId, trainingJobObjective = command.trainingJobObjective, featureSchemaId = command.featureSchemaId, roundId = command.roundId, roundNumber = command.roundNumber, maxRounds = command.maxRounds, minimumAccuracy = command.minimumAccuracy, aggregatedModelId = command.aggregatedModelId, aggregatedModelArtifactUri = command.aggregatedModelArtifactUri, aggregatedModelRegistryRef = command.aggregatedModelRegistryRef, modelFormat = command.modelFormat, modelArtifactDigest = command.modelArtifactDigest, aggregatedModelSignatureUri = command.aggregatedModelSignatureUri, globalAccuracy = portResult.globalAccuracy, globalFairnessScore = portResult.globalFairnessScore))
                }
    }
}
