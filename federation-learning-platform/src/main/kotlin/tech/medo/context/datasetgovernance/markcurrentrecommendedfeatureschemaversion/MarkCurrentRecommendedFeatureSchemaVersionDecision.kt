...
class MarkCurrentRecommendedFeatureSchemaVersionDecision {
    fun decide(command: MarkCurrentRecommendedFeatureSchemaVersionCommand, state: FeatureSchemaState): List<Any> {
        ...
        return listOf(
            CurrentRecommendedFeatureSchemaVersionMarkedEvent(featureSchemaId = command.featureSchemaId, featureDomain = command.featureDomain, recommendedVersion = "" /* TODO: Expose the recommended schema version for catalog projection. */, recommendationNote = command.recommendationNote, version = command.version)
        )
    }
}
