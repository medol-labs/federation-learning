...
class AggregatePlainModelUpdatesCommandHandler(
    ...
    fun handle(
        ...
        val input = AggregatePlainModelUpdatesInput(trainingJobId = command.trainingJobId, trainingRunConfigurationId = command.trainingRunConfigurationId, featureSchemaId = command.featureSchemaId, roundId = command.roundId, roundNumber = command.roundNumber, maxRounds = command.maxRounds, minimumAccuracy = command.minimumAccuracy, aggregationAlgorithm = command.aggregationAlgorithm, aggregatedModelId = command.aggregatedModelId, modelUpdateArtifactRefs = command.modelUpdateArtifactRefs)
