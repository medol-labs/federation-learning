...
class AuditCriticalTrainingEventsProcessor(private val commandGateway: CommandGateway) {
    @EventHandler
    fun on(event: TrainingAlertRaisedEvent): java.util.concurrent.CompletableFuture<AppendAuditTrailCommand> =
        commandGateway.send(AppendAuditTrailCommand(auditRecordId = java.util.UUID.randomUUID() /* TODO: provide auditRecordId */, sourceEventName = "" /* TODO: provide sourceEventName */, sourceEntityId = null /* TODO: provide sourceEntityId */, severity = event.severity, payloadHash = "" /* TODO: provide payloadHash */)).resultMessage.thenApply { it.payload() as AppendAuditTrailCommand }
}
